...
def evaluate_threshold_pilot(
    ...
    *,
    ...
    if any(value not in available for value in normalized_cutoffs):
        raise ValueError(
            "every evaluated cutoff must be present in the artifact threshold ladder"
        )
    normalized_ks = []
    for value in ks:
    ...
    frames = {
        int(record["frame"]): list(record.get("candidates", ())) for record in records
    }
    ...
    for cutoff in normalized_cutoffs:
        ...
        observed = selected = retained_selected = missing = annotated_missing = (
            cutoff_misses
        ) = 0
        ...
        for frame, label in resolved.items():
            ...
            if kind not in {"selected", "candidate"}:
                raise ValueError(
                    f"unsupported pilot label kind at frame {frame}: {kind!r}"
                )
            ...
            selected_group = next(
                (
                    ...
                    if selected_id in group["raw_member_ids"]
                ),
                None,
            )
            if selected_group is None:
                raise ValueError(
                    f"selected candidate {selected_id!r} is absent at frame {frame}"
                )
        ...
        results.append(
            {
                ...
                "recall_at_k": {
                    "all" if k is None else str(k): hits[k] / observed
                    if observed
                    else None
                    for k in normalized_ks
                },
                "hits_at_k": {
                    "all" if k is None else str(k): hits[k] for k in normalized_ks
                },
            }
        )
    return {
...
def freeze_threshold_policy(
    ...
    *,
    minimum_cutoff: float | None = None,
    retention_k: int | None = None,
) -> dict[str, Any]:
    ...
    qualified = [
        row
        for row in rows
        if row.get("observed_proposal_recall") is not None
    ...
    if qualified:
        ...
        chosen = min(
            ...
            key=lambda row: (
                ...
                float(
                    row["grouped_candidates_per_frame"].get(
    ...
    if (minimum_cutoff is None) != (retention_k is None):
        raise ValueError("manual freeze requires both minimum_cutoff and retention_k")
    if minimum_cutoff is not None:
        matches = [
            row
            for row in rows
            if math.isclose(float(row["minimum_cutoff"]), float(minimum_cutoff))
        ]
        if not matches:
            raise ValueError("manual minimum cutoff was not evaluated in this report")
        if isinstance(retention_k, bool) or int(retention_k) < 1:
            raise ValueError("manual retention K must be a positive integer")
        chosen = matches[0]
        available_ks = {int(key) for key in chosen["hits_at_k"] if key != "all"}
        if int(retention_k) not in available_ks:
            raise ValueError("manual retention K was not evaluated in this report")
        frozen_k = int(retention_k)
        reason = "manual_tradeoff_override"
    hit_key = "all" if frozen_k is None else str(frozen_k)
    observed_hits = int(chosen["hits_at_k"][hit_key])
    observed_target_frames = int(chosen["observed_target_frames"])
    observed_proposal_recall = (
        observed_hits / observed_target_frames if observed_target_frames else None
    )
    observed_wilson = wilson_interval(observed_hits, observed_target_frames)
    ...
    return {
        ...
        "observed_target_frames": observed_target_frames,
        "observed_proposal_recall": observed_proposal_recall,
        "wilson_95": observed_wilson,
        "selection_reason": reason,
        "report_fingerprint": report_fingerprint,
        "automatic_recommendation": automatic,
        "user_selected_tradeoff": (
            {"minimum_cutoff": float(chosen["minimum_cutoff"]), "retention_k": frozen_k}
            if reason == "manual_tradeoff_override"
            else None
        ),
...
def filter_pilot_artifact(
    ...
    thresholds = sorted(
        float(value)
        for value in metadata.get("thresholds", ())
    ...
    for record in records:
        ...
        frozen_candidates = sorted(
            group_shuttle_candidates(candidates), key=candidate_retention_key
        )
        if retention_k is not None:
            groups = frozen_candidates
            frozen_candidates = groups[:retention_k]
            retained_ids = {
                candidate_id
                for group in frozen_candidates
                for candidate_id in group["raw_member_ids"]
            }
            candidates = [
                item for item in candidates if item["candidate_id"] in retained_ids
            ]
        lines.append(
            json.dumps(
                {
    ...
    if output_path.exists():
        if output_path.read_bytes() != encoded:
            raise FileExistsError(
                f"production candidate artifact already differs: {output_path}"
            )
        return output_path
...
def validate_artifact_lineage(old_path: str | Path, new_path: str | Path) -> None:
    """Require exact provenance and immutable records inherited from the parent."""
    ...
    if [int(item["frame"]) for item in old_records] != [
        int(item["frame"]) for item in new_records
    ]:
    ...
    frozen_child = "frozen_candidate_view" in new_meta
    if frozen_child and new_meta.get("parent_artifact_sha256") != _sha256(old_path):
        raise ValueError("frozen candidate parent fingerprint mismatch")
    for old_record, new_record in zip(old_records, new_records):
        old_shared = [
            item
            for item in old_record.get("candidates", ())
            if float(item.get("threshold", math.nan)) in shared
        ]
        new_shared = [
            item
            for item in new_record.get("candidates", ())
            if float(item.get("threshold", math.nan)) in shared
        ]
        if frozen_child:
            old_by_id = {str(item.get("candidate_id")): item for item in old_shared}
            inherited_exactly = all(
                old_by_id.get(str(item.get("candidate_id"))) == item
                for item in new_shared
            )
        else:
            inherited_exactly = old_shared == new_shared
        if not inherited_exactly:
            raise ValueError(
...
def _read_events(path: Path) -> tuple[AnnotationEvent, ...]:
    records = [
        json.loads(line) for line in path.read_bytes().splitlines() if line.strip()
    ]
...
def migrate_v1_runtime(
    ...
    *,
    ...
    v1_runtime, pilot_runtime = (
        Path(v1_runtime).resolve(),
        Path(pilot_runtime).resolve(),
    )
    ...
    for source_id, source in pilot_registry.sources.items():
        ...
        validate_artifact_lineage(
            old_artifacts[source_id], source.artifacts["candidates"]
        )
        new_hashes[source_id] = plugin.verify_artifact_fingerprint(source)
        old_source_events = [
            event for event in old_events if event.source_id == source_id
        ]
        if any(
            event.source_video_sha256 != source.video_sha256
            for event in old_source_events
        ):
            raise ValueError(f"source-video fingerprint mismatch for {source_id}")
    queues = [
        AnnotationQueue.read(path)
        for path in sorted((v1_runtime / "queues").glob("*.json"))
    ]
    for queue in queues:
        for burst in queue.bursts:
            ...
            if burst.source_video_sha256 != source.video_sha256:
                raise ValueError(
                    f"queue source-video fingerprint mismatch for {burst.source_id}"
                )
            if burst.candidate_artifact_sha256 != _sha256(
                Path(old_artifacts[burst.source_id])
            ):
                raise ValueError(
                    f"queue candidate fingerprint mismatch for {burst.source_id}"
                )
    ...
    safe = [
        ...
        and (event.source_id, event.frame) not in review
    ]
    ...
    try:
            ...
            replace(
                ...
                construction={
                    **migrated.construction,
                    "audit_queue_id": rebound_audit_ids[
                        original.construction["audit_queue_id"]
                    ],
                },
            )
        ...
        for event in sorted(safe, key=lambda item: (item.source_id, item.frame)):
            ...
            representative = plugin.representative_candidate_id(
                source, event.frame, event.candidate_id or ""
            )
            group = next(
                item
                for item in plugin.annotator_overlays(source, event.frame)
        ...
        manifest = {
            **lineage,
            ...
            "parent_file_sha256": {
                str(path.relative_to(v1_runtime)): digest
...
def materialize_final_runtime(
    ...
    pilot_runtime, final_runtime = (
        Path(pilot_runtime).resolve(),
        Path(final_runtime).resolve(),
    )
    ...
    for source_id, source in final_registry.sources.items():
        ...
        validate_artifact_lineage(
            pilot_artifacts[source_id], source.artifacts["candidates"]
        )
    ...
    try:
        ...
        for event in sorted(
            ...
            if label_kind == "selected":
                pilot_members = set(
                    (event.annotation_metadata or {}).get(
                        "raw_member_ids",
                        (event.candidate_id,) if event.candidate_id else (),
                    )
                )
                ...
                retained = next(
                    (
                        ...
                        if pilot_members.intersection(group["raw_member_ids"])
                    ),
                    None,
                )
                if retained is None:
                    ...
                    derived_missing += 1
                    mapping["derived_reason"] = (
                        "selected_pilot_group_has_no_retained_member"
                    )
                else:
            ...
            if label_kind == "no_shuttle":
                raise ValueError(
                    "legacy no_shuttle must be re-reviewed before final materialization"
                )
            store.record(
                task=event.task,
                source_id=event.source_id,
