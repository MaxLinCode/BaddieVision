...
def test_high_to_low_grouping_prevents_bridges_and_duplicate_threshold_members() -> (
    None
):
    raw = [
    ...
    flattened = [
        candidate_id for group in groups for candidate_id in group["raw_member_ids"]
    ]
    assert sorted(flattened) == sorted(item["candidate_id"] for item in raw)
    assert len(flattened) == len(set(flattened))
    assert next(group for group in groups if group["candidate_id"] == "high-a")[
        "raw_member_ids"
    ] == ["high-a", "low-a2"]
    assert next(group for group in groups if group["candidate_id"] == "high-b")[
        "raw_member_ids"
    ] == ["high-b", "low-bridge"]
    assert all(
        len(
            {
                next(
                    item["threshold"] for item in raw if item["candidate_id"] == member
                )
                for member in group["raw_member_ids"]
            }
        )
        == len(group["raw_member_ids"])
        for group in groups
    )
    assert all(group["grouping_version"] == GROUPING_VERSION for group in groups)


def test_observation_contract_selector_mapping_and_legacy_write_rejection(
    tmp_path: Path,
) -> None:
    ...
    assert [
        (item.kind, item.hotkey) for item in ShuttleSelectionPlugin().label_options
    ] == [
...
def test_pilot_metrics_freeze_and_filter_keep_stable_raw_ids(tmp_path: Path) -> None:
    ...
    report = evaluate_threshold_pilot(
        pilot, labels, cutoffs=(0.1, 0.5), ks=(1, 2, None)
    )
    ...
    metadata, *records = [
        json.loads(line) for line in production.read_text().splitlines()
    ]
    assert (
    ...
    assert [item["candidate_id"] for item in records[0]["candidates"]] == [
        "f0-high",
        "f0-low",
    ]
    assert [item["candidate_id"] for item in records[0]["frozen_candidates"]] == [
        "f0-high"
    ]
    assert records[0]["frozen_candidates"][0]["raw_member_ids"] == [
        "f0-high",
        "f0-low",
    ]
    assert metadata["frozen_candidate_view"]["record_field"] == ("frozen_candidates")
    validate_artifact_lineage(pilot, production)
    assert filter_pilot_artifact(pilot, production, freeze) == production


def test_lineage_validation_rejects_shared_threshold_record_changes(
    tmp_path: Path,
) -> None:
    old = _artifact(
        tmp_path / "old.jsonl",
...
def test_freeze_uses_highest_qualifying_cutoff_and_smallest_equivalent_k() -> None:
    ...
    manual = freeze_threshold_policy(report, minimum_cutoff=0.05, retention_k=2)
    assert manual["selection_reason"] == "manual_tradeoff_override"
    assert manual["automatic_recommendation"]["minimum_cutoff"] == 0.10
    assert manual["user_selected_tradeoff"] == {
        "minimum_cutoff": 0.05,
        "retention_k": 2,
    }
    assert len(manual["report_fingerprint"]) == 64

    constrained = freeze_threshold_policy(report, minimum_cutoff=0.05, retention_k=1)
    assert constrained["observed_hits"] == 98
    assert constrained["observed_target_frames"] == 100
    assert constrained["observed_proposal_recall"] == 0.98
    assert constrained["wilson_95"] == wilson_interval(98, 100)
    with pytest.raises(ValueError, match="both"):
        freeze_threshold_policy(report, minimum_cutoff=0.05)
    with pytest.raises(ValueError, match="not evaluated"):
        freeze_threshold_policy(report, minimum_cutoff=0.07, retention_k=2)
...
def test_v1_migration_is_transactional_preserves_queue_order_and_classifies_labels(
    ...
    audit_queue = AnnotationQueue(
        queue_id="v1-audit-queue",
        kind="audit",
        task="shuttle_selection",
        seed=1729,
        bursts=(burst,),
        construction={"anchor_count": 1},
    )
    ...
    assert {
        path: path.read_bytes() for path in v1.rglob("*") if path.is_file()
    } == before
    rebound = AnnotationQueue.read(output / "queues" / "shuttle-adaptive.json")
    assert rebound.bursts[0].frames == (2, 0, 1)
    assert (
        rebound.bursts[0].candidate_artifact_sha256
        == hashlib.sha256(pilot_candidates.read_bytes()).hexdigest()
    )
    ...
    migrated = [
        json.loads(line)
        for line in (output / "events" / "shuttle.jsonl").read_text().splitlines()
    ]
    assert len(migrated) == 1
    assert migrated[0]["review_action"] == "migrated"
    assert (
        migrated[0]["annotation_metadata"]["migration"]["v1_candidate_id"]
        == "f0-shared"
    )
